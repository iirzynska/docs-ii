...
image::multicloud-gitops-amx-rhoai/amx-rhoai-validated-patter-operator.png[???Install Validated Patterns Operator,scale=50]
+
...
. Fill the *Name* of the pattern `multicloud-gitops-amx-rhoai` and *Cluster Group Name* `hub` (from _values-global.yaml_ file).
...
image::multicloud-gitops-amx/multicloud-gitops-argocd-amx-rhoai.png[Multicloud GitOps Hub with `amx-app`]

== Enablement

== Intel® Advanced Matrix Extensions Instructions

The kernel detects Intel® AMX at run-time, there is no need to enable and configure it additionally to improve performance.

Verify if the *openvino-notebooks-v2022.3-1* build is completed under *Builds* > *Builds*.

In the RHODS console open Jupyter by clicking *Launch application*, choose *OpenVINO™ Toolkit v2022.3* notebook image, *Large* container size and start the notebook server.
image:openvino_toolkit_1.PNG[openvino_toolkit_1]

On the Launcher window choose Notebook with *Python 3 (ipykernel)*.
image:openvino_toolkit_2.PNG[openvino_toolkit_2]

All necessary tools like *Model Downloader* (https://docs.openvino.ai/2022.3/omz_tools_downloader.html#installation) and *Benchmark Python Tool* (https://docs.openvino.ai/2022.3/openvino_inference_engine_tools_benchmark_tool_README.html#doxid-openvino-inference-engine-tools-benchmark-tool-r-e-a-d-m-e) are built in and ready to use.

To start work with notebook set the environment variable to  view compiled instructions and control the usage of AMX:

----
%env ONEDNN_VERBOSE=1
----

Download the BERT-Large model compatible with FP32&BF16 precision (in this case https://docs.openvino.ai/2022.3/omz_models_model_bert_large_uncased_whole_word_masking_squad_0001.html):
...
In ONEDNN verbose you should see *avx_512_core_amx*, what confirms that AMX instructions are being used.

To compare results without AMX you may run this benchmark with different precision f.e.
FP32.
