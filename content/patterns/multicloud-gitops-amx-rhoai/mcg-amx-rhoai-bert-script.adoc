...
== Introduction
The {amx-rhoai-mcg-pattern} provides tools for developers or data scientists to ...
...
. Choose *OpenVINO™ Toolkit v2022.3* notebook image with *Large* container size and start the notebook server. Server launching will take several minutes. Once it is ready, go to access notebook server.
...
. Download BERT-large example, that uses AMX accelerator, by typing in the opened notebook:
	...
	!wget https://raw.githubusercontent.com/iirzynska/multicloud-gitops-ii/intel-rhods-amx/common/scripts/BERT.ipynb -O BERT.ipynb
	

. On the left-hand side menu the `BERT.ipynb` script should show up. Open it and run instructions one by one with `Ctr+Enter`
...
To compare results without AMX you may run this benchmark with different precision i.e. FP32.
----
!benchmark_app -m bert-large-uncased-whole-word-masking-squad-0001.xml -infer_precision f32
----
