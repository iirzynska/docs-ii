...
image::multicloud-gitops-amx-rhoai/amx-rhoai-logical-diagram.png[Logical Architecture]
...
* OpenVINO Toolkit Operator manages OpenVINO components within Openshift enviroment. First one, OpenVINO™ Model Server (OVMS) is a scalable, high-performance solution for serving machine learning models optimized for Intel® architectures. The other component, that was used in the proposed pattern is Notebook resource. This element integrates JupyterHub from OpenShift AI with a container image that includes developer tools from the OpenVINO toolkit. It also enables selecting a defined image OpenVINO™ Toolkit from the Jupyter Spawner choice list.
...
// image::multicloud-gitops-amx-rhoai/amx-rhoai-app-log.png[Logs from amx-app pod]
