...
As an example of AI workload in the pattern serves BERT-Large model. It is a wide known model used by various enterprise Natural Language Processing workloads. Intel has demonstrated, that *{intel-5th-gen-xeon-processors}* perform up to *1.49* times better in NLP flows vs. prior generation:
